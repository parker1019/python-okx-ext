...
class AccountAPI(OkxClient):
    ...
    def get_account_bills(self, instType='', instId='', ccy='', mgnMode='', ctType='', type='', subType='', after='', before='',
                          begin='', end='', limit=''):
        params = {'instType': instType, 'instId': instId, 'ccy': ccy, 'mgnMode': mgnMode, 'ctType': ctType, 'type': type,
                  'subType': subType, 'after': after, 'before': before, 'begin': begin, 'end': end, 'limit': limit}
        return self._request_with_params(GET, BILLS_DETAIL, params)

    # Get Bills Details (recent 3 months)
    def get_account_bills_archive(self, instType='', instId='', ccy='', mgnMode='', ctType='', type='', subType='', after='',
    ...
    def get_max_avail_size(self, instId, tdMode, ccy='', reduceOnly='', px=''):
        params = {'instId': instId, 'tdMode': tdMode, 'ccy': ccy, 'reduceOnly': reduceOnly, 'px': px}
        return self._request_with_params(GET, MAX_AVAIL_SIZE, params)

    # Increase / Decrease margin
    def adjustment_margin(self, instId, posSide, type, amt, ccy=''):
        params = {'instId': instId, 'posSide': posSide, 'type': type, 'amt': amt, 'ccy': ccy}
    ...
    def get_max_loan(self, mgnMode, instId='', ccy='', mgnCcy=''):
        params = {'mgnMode': mgnMode, 'instId': instId, 'ccy': ccy, 'mgnCcy': mgnCcy}
    ...
    def get_interest_accrued(self, type='', instId='', ccy='', mgnMode='', after='', before='', limit=''):
    ...
    # Position Builder
    def position_builder(self, acctLv='', inclRealPosAndEq=True, lever='', simPos=None, simAsset=None, greeksType='BS'):
        params = {'acctLv': acctLv, 'inclRealPosAndEq': inclRealPosAndEq, 'lever': lever, 'greeksType': greeksType}
